...
> 在线试运行：[`http://pluviohan.com`](http://pluviohan.com)
...
| 前端 | Next.js + TailwindCSS | 响应式 Web 看板 |
| 部署 | Nginx + Systemd + DNSPod | 云端生产级反向代理 + 服务自启 + DNS 解析 |
...
- ✅ 自动监测守护进程（含图表生成与告警推送）
- ✅ Next.js 前端业主看板原型
- ✅ Nginx + Systemd 云端生产级部署
- ✅ 8 小时连续耐久测试（2026-02-19，96 个采样点零错包、零丢失）
- ✅ 独立域名 `pluviohan.com` 公网试运行
...
[`http://pluviohan.com`](http://pluviohan.com) — 项目原型已通过独立域名完成公网试运行，可对外展示完整运行链路。
